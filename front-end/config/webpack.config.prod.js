...
// requires a trailing slash, or the file assets will get an incorrect path.
const publicPath = paths.servedPath;
// Some apps do not use client-side routing with pushState. For these,
...
// trailing slash as %PUBLIC_URL%/xyz looks better than %PUBLIC_URL%xyz.
const publicUrl = publicPath.slice(0, -1);
// Get environment variables to inject into our app.
const env = getClientEnvironment(publicUrl);

// Assert this just to be safe. Development builds of React are slow and not
...
// ExtractTextPlugin expects the build output to be flat. (See
// https://github.com/webpack-contrib/extract-text-webpack-plugin/issues/27)
// However, our output is structured with css, js and media folders. To have
// this structure working with relative paths, we have to use custom options.
...
// This is the production configuration. It compiles slowly and is focused on
// producing a fast and minimal bundle. The development configuration is
// different and lives in a separate file.
module.exports = {
  ...
  output: {
    // The build folder.
    path: paths.appBuild,
    // Generated JS file names (with nested folders). There will be one main bundle,
    // and one file per asynchronous chunk. We don't currently advertise code
    // splitting but Webpack supports it.
  ...
  resolve: {
    ...
    // component filename extension to support some tools, although we do not
  ...
  module: {
    ...
    rules: [
      ...
      // before Babel processes the JS.
      {
        ...
        oneOf: [
          // "url" loader works just like "file" loader but it also embeds assets smaller
          // than specified size as data URLs to avoid requests.
          {
            test: [
              /\.bmp$/, /\.gif$/, /\.jpe?g$/, /\.png$/
            ],
          ...
          // The notation here is somewhat confusing. "postcss" loader applies
          // autoprefixer to our CSS. "css" loader resolves paths in CSS and adds assets
          // as dependencies. "style" loader normally turns CSS into JS modules injecting
          // <style>, but unlike in development configuration, we do something different.
          // `ExtractTextPlugin` first applies the "postcss" and "css" loaders (second
          // argument), then grabs the result CSS and puts it into a separate file in our
          // build process. This way we actually ship a single CSS file in production
          // instead of JS code injecting <style> tags. If you use code splitting,
          // however, any async bundles will still use the "style" loader inside the async
  ...
  plugins: [
    ...
    // href="%PUBLIC_URL%/favicon.ico"> In production, it will be an empty string
    // unless you specify "homepage" in `package.json`, in which case it will be the
    // pathname of that URL.
    ...
    // (process.env.NODE_ENV === 'production') { ... }. See `./env.js`. It is
    // absolutely essential that NODE_ENV was set to production here. Otherwise
    // React will be compiled in the very slow development mode.
    ...
    new webpack
      ...
      .UglifyJsPlugin({
        compress: {
          ...
          // https://github.com/facebookincubator/create-react-app/issues/2376 Pending
          // further investigation: https://github.com/mishoo/UglifyJS2/issues/2011
          comparisons: false
        },
        mangle: {
          safari10: true
    ...
    // Generate a manifest file which contains a mapping of all asset filenames to
    // their corresponding output file so that tools can pick it up without having
    // to parse `index.html`.
    new ManifestPlugin({fileName: 'asset-manifest.json'}),
    // Generate a service worker script that will precache, and keep up to date, the
    // HTML & assets that are part of the Webpack build.
    new SWPrecacheWebpackPlugin({
      // By default, a cache-busting query parameter is appended to requests used to
      // populate the caches, to ensure the responses are fresh. If a URL is already
      // hashed by Webpack, then there is no concern about it being stale, and the
      // cache-busting can be skipped.
      ...
      // https://github.com/facebookincubator/create-react-app/issues/2237#issuecommen
      // t -302693219
